...
### 実装に必要な作業
- [x] モデル選択ダイアログコンポーネント作成
- [x] モデル一覧取得ロジック
- [ ] お気に入り/最近の永続化
- [x] キーバインド接続（`Ctrl+M`で開く）
...
### 実装に必要な作業
- [x] `App::new()`でのハードエラーを削除
- [x] モデル未設定状態でTUI起動を許可
- [x] 初期表示でプロバイダー/モデル設定ダイアログを表示
...
### 実装に必要な作業
- [x] `Action::ModelSelector`をダイアログに接続（`Ctrl+M`で開く）
- [ ] 設定からのキーバインド読み込み
- [ ] リーダーキーサポート
...
### Rust（修正対象）
- `src/tui/app.rs` - ハードエラー削除、ダイアログ状態追加 (完了)
- `src/tui/components.rs` - ダイアログコンポーネント追加
- `src/tui/input.rs` - ModelSelector アクション接続 (完了)
- `src/tui/ui.rs` - ダイアログレンダリング (完了)
- `src/provider/mod.rs` - 認証ロード追加 (完了)
- `src/auth.rs` - 認証ストレージモジュール (完了)
